...
def fetch_data_from_postgres():
    try:
        ...
        connection = psycopg2.connect(host=PG_HOST,port=PG_PORT,dbname=PG_DATABASE,user=PG_USER,password=PG_PASSWORD)
...
def create_nodes_and_relationships_in_neo4j(person_df, movie_df, acted_in_df):
    try:
        ...
        logging.info("Successfully created Person and Movie nodes, and Acted_in relationships in Neo4j.")
    except Exception as e:
        logging.error(f"Error creating nodes and relationships in Neo4j: {e}")
        raise
